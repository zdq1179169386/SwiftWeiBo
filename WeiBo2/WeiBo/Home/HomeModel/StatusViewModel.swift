...
class StatusViewModel: NSObject {
    ...
    init(status : Status) {
        ...
        if let created_at = status.created_at{
            createText = Date.creatDateStr(created_at)
        }
        //处理来源字符串
        if let source = status.source , source != ""{
            ...
            sourceText = "来自" + (source as NSString).substring(with: NSMakeRange(startIndex, length))
        }
...
//                print("22222\(picUrlStr)")
                picURLs.append(URL.init(string: picUrlStr)!)
//                print("3333")
