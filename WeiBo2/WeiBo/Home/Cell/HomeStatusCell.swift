...
@objc protocol HomeStatusCellDelegate: NSObjectProtocol {
    ...
    @objc optional func HomeStatusCellTextClick(_ text : String,range : NSRange)
}
...
//            print("---\(viewModel.status?.user?.screen_name),\(viewModel.status?.text),\(viewModel.status?.retweeted_status?.text)")
            //设置头像
            iconView.sd_setImage(with: viewModel.iconURL, placeholderImage: UIImage.init(named: "avatar_default_small") )
            //设置认证图标
            verfiedView.image = viewModel.verifiedImage
            //设置昵称
            screenNameLabel.text = viewModel.status?.user?.screen_name
            screenNameLabel.textColor = viewModel.vipImage == nil ? UIColor.orange : UIColor.black
            //vip
            ...
            if viewModel.status?.retweeted_status != nil {
                if let screenNmae = viewModel.status?.retweeted_status?.user?.screen_name , let retweentedText = viewModel.status?.retweeted_status?.text {
                    ...
                    //设置转发背景的隐藏
                    retweentedBGView.isHidden = false
                    //设置转发微博正文top约束
                ...
                //设置转发背景的隐藏
                retweentedBGView.isHidden = true
                //设置转发微博正文top约束
...
extension HomeStatusCell {
    fileprivate func calculatePicViewSize(_ count : Int) -> CGSize{
        ...
        //4张配图
        let itemWH = (UIScreen.main.bounds.width - 2 * Margin - 2 * ItemMargin) / 3
         //设置item的大小
